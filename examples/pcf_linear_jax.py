...
def main(argv=None):
    ...
    if args.shearpy:
        solver = PlaneCouetteLinear.shearpy(
            nx=args.nx, Re=args.Re, Rm=args.Rm or args.Re, by=args.by, bz=args.bz,
            magnetic_bc=args.magnetic_bc,
        )
    else:
        solver = PlaneCouetteLinear.couette(
            nx=args.nx, Re=args.Re, Rm=args.Rm, mhd=args.mhd, by=args.by, bz=args.bz,
            magnetic_bc=args.magnetic_bc,
        )

    if args.nonmodal:
        rows = solver.nonmodal_growth(
            args.ky, args.kz, parse_times(args.times), n_modes=args.n_modes,
            energy=args.energy,
        )
