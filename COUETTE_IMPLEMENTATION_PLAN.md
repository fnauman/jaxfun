...
- **PCF-MHD** (`pcf_mhd_jax.py`, `pcf_mhd_mri_shearpy_jax.py`): A-diffusion + compatible-space curl projections; `divB ≈ 2e-16` (weak). The divergence-free PCF-MHD path now has live `shenfun` 1/5/50-step diagnostic and mapped `(u,g,A)` coefficient parity; the shearpy/MRI transport extension still only asserts finite stresses.
...
**B. Validation gaps:** live `shenfun` parity now covers PCF IMEXRK222 diagnostics/physical velocity/coefficient fields at 1/5/50 steps, divergence-free PCF-MHD diagnostics/coefficient fields at 1/5/50 steps, TC linear/MRI stability, and the radial dealiased product. Broader live parity for the PCF-MHD shearpy/MRI transport path and Taylor-Couette DNS trajectories remains open for T4.8, T5.3, T5.4, T6.2, T6.4, and T6.6.
