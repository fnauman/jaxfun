...
| `pcf_mhd_divfree` | pcf | mhd | `examples/pcf_mri_primitive_jax.py` | production runner wired; bounded start/smoke-tier smoke tested and labeled `bounded_saturation_smoke`; full Phase J5 `N=(32,64,32)` float32 GPU candidate decayed and is retained as a failed generated-saturation candidate, not promoted | rung 3 |
...
- `generated_saturated_golden`: full saturation execution without bounded smoke overrides; if `saturation_check_passed` is missing or false, the runner marks metadata failed and exits nonzero instead of writing a passing full-saturation artifact.
