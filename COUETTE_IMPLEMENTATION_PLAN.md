...
> - **Remaining gaps are validation hardening or optional features**, not first ports of the Couette variants: optional KMM `compute_pressure`, live file-output parity, future indefinite saddle-block solver validation, strict bit-identical multi-device parity, and any remaining low-level stencil/operator cross-checks beyond the live TC linear block matrices.
