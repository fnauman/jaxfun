...
from taylor_couette_dns import (AxisymmetricTCDNS, TaylorCouetteDNS,
                                 AxisymmetricMRIDNS, TaylorCouetteMRIDNS)
from taylor_couette_mri import TaylorCouetteMRI
from shenfun import Array, Function, Dx, inner, project
...
def test_laminar_fixed_point_stays_zero():
    ...
    dns = AxisymmetricTCDNS(base, nu=1e-2, Nr=24, Nz=8, Lz=2.0, dt=5e-3,
                            dealias=1.0)
    dns.u_hat[:] = 0.0
    dns.run(0.1)
...
def test_stable_regime_perturbation_decays():
    ...
    dns = AxisymmetricTCDNS(base, nu=1e-2, Nr=32, Nz=8, Lz=2.0, dt=2e-3,
                            dealias=1.0)
...
def test_linear_growth_matches_eigensolver(nu, kz):
    base = CircularCouette(1.0, 2.0, 1.0, 0.0)
    Lz = 2 * math.pi / kz
    dns = AxisymmetricTCDNS(base, nu=nu, Nr=40, Nz=8, Lz=Lz, dt=1e-3,
                            dealias=1.0)
...
def test_incompressibility_to_roundoff():
    ...
    base = CircularCouette(1.0, 2.0, 1.0, 0.0)
    for Nr in (24, 40):
        dns = AxisymmetricTCDNS(base, nu=1e-2, Nr=Nr, Nz=8, Lz=2.0, dt=1e-3,
                                dealias=1.0)
...
def test_3d_axisymmetric_mode_matches_linear():
    ...
    base = CircularCouette(1.0, 2.0, 1.0, 0.0)
    kz = 3.13
    dns = TaylorCouetteDNS(base, nu=1e-2, Nr=40, Ntheta=4, Nz=8,
                           Lz=2 * math.pi / kz, dt=1e-3, dealias=1.0)
...
def test_3d_incompressible():
    ...
    divs = {}
    for Nr in (32, 48):
        dns = TaylorCouetteDNS(base, nu=1e-2, Nr=Nr, Ntheta=8, Nz=8, Lz=2.0,
                               dt=1e-3, dealias=1.0)
...
def test_3d_nonaxisymmetric_growth_matches_linear():
    ...
    base = CircularCouette(1.0, 2.0, 1.0, 0.0)
    nu, kz = 6e-3, 3.0
    dns = TaylorCouetteDNS(base, nu=nu, Nr=36, Ntheta=4, Nz=8,
                           Lz=2 * math.pi / kz, dt=1e-3, dealias=1.0)
...
def test_mri_growth_matches_eigensolver():
    ...
    dns = AxisymmetricMRIDNS(base, B0=B0, nu=nu, eta_mag=eta_mag, Nr=40, Nz=8,
                             Lz=2 * math.pi / kz, dt=2e-3, dealias=1.0)
...
def test_mri_random_seed_is_solenoidal():
    ...
    base = _kep_base()
    dns = AxisymmetricMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=32, Nz=8,
                             Lz=2.0, dt=2e-3, dealias=1.0)
...
def test_mri_requires_field_keplerian_stable_without_B0():
    ...
    base = _kep_base()
    dns = AxisymmetricMRIDNS(base, B0=0.0, nu=1e-3, eta_mag=1e-3, Nr=32, Nz=8,
                             Lz=2.0, dt=2e-3, dealias=1.0)
...
def test_mri_solenoidal_and_nonlinear_finite():
    ...
    keeps div(b) small."""
    base = _kep_base()
    dns = AxisymmetricMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=40, Nz=16,
                             Lz=2 * math.pi / 6.0, dt=2e-3, dealias=1.5)
...
def test_mri_nonlinear_alfvenic_cancellation():
    ...
    base = _kep_base()
    dns = AxisymmetricMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=24, Nz=8,
                             Lz=2.0, dt=2e-3, dealias=1.0)
...
def test_mri_axisymmetric_toroidal_curvature_signs():
    ...
    np.testing.assert_allclose(np.asarray(out[0]), np.asarray(expected), atol=1e-13, rtol=1e-12)
...
def test_mri3d_emf_curl_theta_metric_signs():
    ...
    base = _kep_base()
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=18,
                              Ntheta=6, Nz=8, Lz=2.0, dt=2e-3, dealias=1.0)
    th, zz, rr = dns.TD.local_mesh(True)
    wall = np.sin(np.pi * (rr - base.R1) / base.gap) ** 2
    dns.x[:] = 0.0
    dns.x[0] = _hat_from_values(dns.TD, 1e-3 * wall * np.cos(th) * np.cos(2 * np.pi * zz / dns.Lz))
    dns.x[4] = _hat_from_values(dns.Tbt, 1e-3 * wall * np.sin(th) * np.cos(2 * np.pi * zz / dns.Lz))
    ...
    np.testing.assert_allclose(np.asarray(out[3]), np.asarray(expected_br), atol=1e-13, rtol=1e-12)
    np.testing.assert_allclose(np.asarray(out[4]), np.asarray(expected_bt), atol=1e-13, rtol=1e-12)
    np.testing.assert_allclose(np.asarray(out[5]), np.asarray(expected_bz), atol=1e-13, rtol=1e-12)
...
def test_mri_nonlinear_saturation():
    ...
    base = _kep_base()
    B0, nu, eta_mag, kz = 0.1, 1e-3, 1e-3, 6.0
    dns = AxisymmetricMRIDNS(base, B0=B0, nu=nu, eta_mag=eta_mag, Nr=40, Nz=24,
                             Lz=2 * math.pi / kz, dt=2e-3, dealias=1.5)
    dns.seed_linear_eigenmode(kz_mode=1, amp=1e-4)
    E_hist = []
    d = None
    for _ in range(16):
...
def test_supercritical_saturation():
    ...
    base = CircularCouette(1.0, 2.0, 1.0, 0.0)
    kz = 3.13
    dns = AxisymmetricTCDNS(base, nu=1e-2, Nr=40, Nz=16, Lz=2 * math.pi / kz,
                            dt=4e-3, dealias=1.5)
    dns.seed_linear_eigenmode(kz_mode=1, amp=1e-4)
    E_early = None
    E_hist = []
...
def _mri3d_growth(m, kz=6.0, Nr=28, Ntheta=None, Nz=12, t_skip=0.5, t_meas=1.5):
    ...
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=Nr,
                              Ntheta=Ntheta, Nz=Nz, Lz=2 * math.pi / kz, dt=2e-3,
                              dealias=1.5)
...
def test_mri3d_reduces_to_axisymmetric():
    ...
    base = _kep()
    ax = AxisymmetricMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=28, Nz=12,
                            Lz=2 * math.pi / 6.0, dt=2e-3, dealias=1.5)
    sa = ax.seed_linear_eigenmode(kz_mode=1, amp=1e-7)
    a1 = ax.run(0.5); a2 = ax.run(1.5)
...
def test_mri3d_eigenmode_is_solenoidal():
    ...
    base = _kep()
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=28,
                              Ntheta=6, Nz=12, Lz=2 * math.pi / 6.0, dt=2e-3,
                              dealias=1.5)
...
def test_mri3d_random_seed_is_divfree_and_keeps_div_b_small():
    ...
    through the run."""
    base = _kep()
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=24,
                              Ntheta=6, Nz=12, Lz=2.0, dt=2e-3, dealias=1.5)
    dns.set_random(amp=1e-3, seed=0)
    du0, db0 = dns.divergences()
    f = dns.fields_physical()
    assert float(np.abs(np.asarray(f[0])).max()) > 1e-5      # velocity seeded
    assert float(max(np.abs(np.asarray(f[3])).max(),
                     np.abs(np.asarray(f[4])).max(),
                     np.abs(np.asarray(f[5])).max())) == 0.0  # b starts at zero
...
def test_mri3d_rejects_underresolved_azimuthal_mode():
    ...
    base = _kep()
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=16,
                              Ntheta=2, Nz=8, Lz=2.0, dt=2e-3)
...
def test_mri3d_keplerian_stable_without_field():
    ...
    base = _kep()
    dns = TaylorCouetteMRIDNS(base, B0=0.0, nu=1e-3, eta_mag=1e-3, Nr=24,
                              Ntheta=6, Nz=12, Lz=2.0, dt=2e-3, dealias=1.5)
...
def test_mri3d_nonlinear_saturation_nonaxisymmetric():
    ...
    base = _kep()
    dns = TaylorCouetteMRIDNS(base, B0=0.1, nu=1e-3, eta_mag=1e-3, Nr=28,
                              Ntheta=8, Nz=14, Lz=2 * math.pi / 6.0, dt=2e-3,
                              dealias=1.5)
