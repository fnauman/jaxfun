...
class PlaneCouetteFluctuationDiagnostics(KMM):
    def __init__(self,
        ...
        KMM.__init__(self, N=N, domain=domain, nu=nu, dt=dt, conv=conv,
                     ...
                     padding_factor=padding_factor, modplot=-1,
    ...
    def print_diagnostics(self, t, tstep):
        ...
        wall_fluct = max(float(np.max(np.abs(ubp[:, 0, :, :]))),
...
def _parse_cli(params):
    parser = argparse.ArgumentParser(description='Plane Couette fluctuations div(u) diagnostics')
    ...
    parser.add_argument('--perturbation-amplitude', type=float, default=params['perturbation_amplitude'])
