...
> - **Remaining gap is validation hardening**, not a first port of any Couette variant: strict bit-identical multi-device parity. Existing multi-device tests cover roundoff-level parity; exact bit parity remains open.
...
- **Current coverage:** `tests/couette/test_sharding_parity_jax.py` covers two-device TC diagnostics and physical transforms for axisymmetric/full-3D hydro and axisymmetric/full-3D MHD states on a Fourier-sharded axis; it now also checks one-step and five-step coefficient/diagnostic parity for all four TC DNS quadrants to roundoff. Concrete multi-device `scan_steps` states use an eager loop so sharded transform paths stay concrete while normal single-device solves keep the `lax.scan` path.
...
**B. Validation gaps:** live `shenfun` parity now covers PCF IMEXRK222 diagnostics/physical velocity/coefficient fields at 1/5/50 steps, both PCF-MHD variants' diagnostics/coefficient fields at 1/5/50 steps, axisymmetric and full-3D hydro/MHD TC DNS diagnostics/coefficient fields at 1/5/50/100 steps, TC linear and MRI raw operator block matrices, TC linear/MRI eigenvalues plus non-modal growth, and the radial dealiased product. Remaining validation work is broader-size/dealiased production coverage and strict bit-identical multi-device parity; current multi-device tests are roundoff-level.
