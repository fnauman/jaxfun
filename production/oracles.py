...
def _tc_inner_torque(solver: Any, state: Any) -> float:
    ...
    nu = float(solver.nu)
    radial = solver.TD.basespaces[-1]
    bounds = jnp.asarray(radial.domain, dtype=jnp.real(state.u[1]).dtype)
    reference_bounds = radial.map_reference_domain(bounds)
    value_rows = radial.evaluate_basis_derivative(reference_bounds, 0)
    derivative_rows = radial.evaluate_basis_derivative(reference_bounds, 1)
    derivative_rows = derivative_rows * float(radial.domain_factor)
    ut_inner_modes = jnp.einsum("i,...i->...", value_rows[0], state.u[1])
    dut_inner_modes = jnp.einsum("i,...i->...", derivative_rows[0], state.u[1])
    ut_inner = solver.F.backward(ut_inner_modes)
    dut_inner = solver.F.backward(dut_inner_modes)
    perturbation_shear = jnp.mean(dut_inner - ut_inner / r_inner)
