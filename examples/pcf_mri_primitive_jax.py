"""Axisymmetric primitive-b plane-Couette / MRI DNS in jaxfun.

This is the JAX counterpart of ``couette/pcf_mri_primitive.py`` for the
``ky=0`` channel-mode DNS goldens.  It evolves primitive velocity and magnetic
field components directly, not a vector potential, so the linear eigenmode from
``examples.pcf_linear_jax.PlaneCouetteLinear`` can be injected block-for-block.
"""
