...
class KMM:
    ...
    def __init__(
        ...
        self.curly = Project(
            curl(self.u_)[1], self.TC, output_array=self.curl[1]
        )  # curlx is already in g
        ...
        self.checkpoint = Checkpoint(
            filename, checkevery=checkpoint, data={"0": {"U": [self.u_]}}
        )
    ...
    def convection(self):
        ...
        if self.conv == 0:
            dudxp = self.dudxp = (
                self.dudx().backward(padding_factor=self.padding_factor).v
            )
            dudyp = self.dudyp = (
                self.dudy().backward(padding_factor=self.padding_factor).v
            )
            dudzp = self.dudzp = (
                self.dudz().backward(padding_factor=self.padding_factor).v
            )
            dvdxp = self.dvdxp = (
                self.dvdx().backward(padding_factor=self.padding_factor).v
            )
            dvdyp = self.dvdyp = (
                self.dvdy().backward(padding_factor=self.padding_factor).v
            )
            dvdzp = self.dvdzp = (
                self.dvdz().backward(padding_factor=self.padding_factor).v
            )
            dwdxp = self.dwdxp = (
                self.dwdx().backward(padding_factor=self.padding_factor).v
            )
            dwdyp = self.dwdyp = (
                self.dwdy().backward(padding_factor=self.padding_factor).v
            )
            dwdzp = self.dwdzp = (
                self.dwdz().backward(padding_factor=self.padding_factor).v
            )
    ...
    def compute_pressure(self):
        if self.solP is None:
            ...
            N0 = self.N0 = FunctionSpace(
                self.N[0],
                self.B0.family(),
                bc={"left": {"N": d2udx2}, "right": {"N": d2udx2}},
            )
    ...
    def print_energy_and_divergence(self, t, tstep):
        if tstep % self.moderror == 0 and self.moderror > 0:
            ...
            if comm.Get_rank() == 0:
                print(
                    "Time %2.5f Energy %2.6e %2.6e %2.6e div %2.6e"
                    % (t, e0, e1, e2, e3)
                )
