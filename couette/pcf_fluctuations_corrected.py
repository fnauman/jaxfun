...
class PlaneCouetteFluctuation(KMM):
    def __init__(self,
        ...
        KMM.__init__(self, N=N, domain=domain, nu=nu, dt=dt, conv=conv,
                     dpdy=0.0, filename=filename, family=family,
                     padding_factor=padding_factor, modplot=modplot,
                     modsave=modsave, moderror=moderror, checkpoint=checkpoint,
    ...
    def initialize(self, from_checkpoint: bool = False):
        ...
        if self.perturbation_amplitude > 0:
            ...
            U[0] += amp*wall*np.sin(2*np.pi*X[1]/self.F1.domain[1])*np.cos(2*np.pi*X[2]/self.F2.domain[1])
            U[1] += amp*wall*np.cos(2*np.pi*X[1]/self.F1.domain[1])*np.sin(2*np.pi*X[2]/self.F2.domain[1])
            U[2] += amp*wall*np.sin(4*np.pi*X[1]/self.F1.domain[1])*np.cos(4*np.pi*X[2]/self.F2.domain[1])
    ...
    def print_energy_and_divergence(self, t, tstep):
        ...
        Epert = inner(1, ubp[0]*ubp[0]) + inner(1, ubp[1]*ubp[1]) + inner(1, ubp[2]*ubp[2])
        ubt = self.total_velocity_physical_from(ubp)
        Etot = inner(1, ubt[0]*ubt[0]) + inner(1, ubt[1]*ubt[1]) + inner(1, ubt[2]*ubt[2])
        ...
        if comm.Get_rank() == 0:
            print(f"t={t:7.3f} Epert={Epert:10.6e} Etot={Etot:10.6e} div={divL2:8.2e} "
    ...
    def plot(self, t, tstep):
        ...
        self.ax1.contourf(self.X[1][:, :, 0], self.X[0][:, :, 0], ubt[1, :, :, 0], 50, cmap='RdBu_r')
        # Streamwise fluctuation in the same plane
        self.ax2.contourf(self.X[1][:, :, 0], self.X[0][:, :, 0], ubp[1, :, :, 0], 50, cmap='RdBu_r')
        # Spanwise fluctuation in an x-z plane at mid y
        mid_y = ubp.shape[2]//2
        self.ax3.contourf(self.X[2][:, 0, :], self.X[0][:, 0, :], ubp[2, :, mid_y, :], 50, cmap='RdBu_r')
        # Energy history
        Epert = inner(1, ubp[0]*ubp[0]) + inner(1, ubp[1]*ubp[1]) + inner(1, ubp[2]*ubp[2])
        Etot = inner(1, ubt[0]*ubt[0]) + inner(1, ubt[1]*ubt[1]) + inner(1, ubt[2]*ubt[2])
    ...
    def init_analysis_plots(self):
        ...
        if not self.enable_live_plots:
            plt.switch_backend('Agg')
        self.fig_analysis, ((self.ax_mean, self.ax_shear), (self.ax_rms, self.ax_energy)) = plt.subplots(2, 2, figsize=(12, 8))
    ...
    def _compute_spectra(self, uhat):
        ...
        energy_density = (np.abs(uhat[0])**2 + np.abs(uhat[1])**2 + np.abs(uhat[2])**2)
        ...
        spec_ky_local = self._fold_spectrum(ky_local, energy_ky_local, ky_max, fold_negative=True)
        spec_kz_local = self._fold_spectrum(kz_local, energy_kz_local, kz_max, fold_negative=False)
    ...
    def init_ssp_plots(self):
        ...
        if not self.enable_live_plots:
            plt.switch_backend('Agg')
        self.fig_ssp, (self.ax_streaks, self.ax_rolls) = plt.subplots(1, 2, figsize=(12, 4))
    ...
    def plot_ssp(self, t, tstep):
        if self.conv != 0 or (not self.save_ssp and not self.enable_live_plots) or self.modssp <= 0 or tstep % self.modssp != 0:
            return
        ubp = self.u_.backward(self.ub)
        ubt = self.total_velocity_physical_from(ubp)
        ...
        s = self.ssp_stride
        self.ax_rolls.quiver(Z[::s, ::s], X[::s, ::s], w_bar[::s, ::s], u_bar[::s, ::s],
                             color='k', scale=50)
        plt.tight_layout()
        if self.save_ssp:
    ...
    def analysis(self, t, tstep):
        if (not self.save_analysis and not self.enable_live_plots) or self.modanalysis <= 0 or tstep % self.modanalysis != 0:
            return
        ...
        self.ax_shear.axhline(self.dUb_dx, color='k', linestyle='--', label='base shear')
        if self.analysis_count > 1:
            self.ax_shear.plot(x_full, self.mean_shear_avg, 'r:', label='time-avg shear')
        ...
        self.ax_energy.plot(self.analysis_time_hist, self.analysis_Epert_hist, 'g-', label="E' (pert)")
        self.ax_energy.plot(self.analysis_time_hist, self.analysis_Etot_hist, 'b-', label='E_total')
    ...
    def spectra(self, t, tstep):
        if (not self.save_spectra and not self.enable_live_plots) or self.modspectra <= 0 or tstep % self.modspectra != 0:
            return
...
def _parse_cli(params):
    parser = argparse.ArgumentParser(description='Plane Couette fluctuations (corrected) diagnostics')
    parser.add_argument('--modplot', type=int, default=params['modplot'], help='Frequency for 2D fields plot')
    parser.add_argument('--modanalysis', type=int, default=params['modanalysis'], help='Frequency for profile analysis plot')
    parser.add_argument('--modspectra', type=int, default=params['modspectra'], help='Frequency for spectra plot')
    parser.add_argument('--modssp', type=int, default=params['modssp'], help='Frequency for SSP (rolls/streaks) plot')
    parser.add_argument('--save-plots', action=argparse.BooleanOptionalAction, default=params['save_plots'])
    parser.add_argument('--save-analysis', action=argparse.BooleanOptionalAction, default=params['save_analysis'])
    parser.add_argument('--save-spectra', action=argparse.BooleanOptionalAction, default=params['save_spectra'])
    parser.add_argument('--save-ssp', action=argparse.BooleanOptionalAction, default=params['save_ssp'])
    parser.add_argument('--enable-live-plots', action=argparse.BooleanOptionalAction, default=params['enable_live_plots'])
